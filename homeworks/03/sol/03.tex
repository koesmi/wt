...
Entsprechend dem Tipp zeigen wir, dass $(Y_n)_n$ gleichgradig integrierbar ist, also, dass $\lim_{k}\sup_{n}E[|Y_n|\mathbbm{1}_{|Y_n|>k}]=0$.
Nach Theorem 22 gilt dann $Y_n\xrightarrow{\mathcal{L}_1}Y$, also $E[Y_n]\to E[Y]$.
Da $E[X_n]\to E[X]$ und $E[Z_n]\to E[Z]$, sind $(X_n)$ und $(Z_n)$ gleichgradig integrierbar.
Damit gilt $\lim_{k}\sup_{n}E[|X_n|\mathbbm{1}_{|X_n|>k}]=\lim_{k}\sup_{n}E[|Z_n|\mathbbm{1}_{|Z_n|>k}]=0$.
Da $X_n\leq Y_n\leq Z_n$ ist $|Y_n|\leq|X_n|+|Z_n|$ für alle $n$.
Mit der Monotonie des Integrals können wir also abschätzen, dass
\begin{align*}
  \lim_{k}\sup_{n}E[|Y_n|\mathbbm{1}_{|Y_n|>k}]
  &\leq\lim_k\sup_n E[|X_n|+|Z_n|]\mathbbm{1}_{|X_n|+|Z_n|\geq k}
\end{align*}
