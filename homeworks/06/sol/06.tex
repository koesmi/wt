...
\emph{Es ist nicht ganz klar, wie man die einzelnen $Y_i$ für das schwache Gesetz der großen Zahlen kriegt.
  Die $X_i^{(n)}$ haben für die ersten paar $n$ folgende Struktur.}
\begin{align*}
  n=1\colon&X_1^{(1)}\\
  n=2\colon&X_1^{(2)},X_2^{(2)}\\
  n=3\colon&X_1^{(3)},X_2^{(3)},X_3^{(3)}\\
  \vdots&
\end{align*}
\emph{Es ist irgendwie nicht klar, dass $\lim_{k\to\infty}X_n^{(k)}$ existiert, beziehungsweise, wie man an dieses kommt.}

\emph{Eine Idee könnte sein, die Maximalungleichung aus Satz 42 zu verwenden.}
